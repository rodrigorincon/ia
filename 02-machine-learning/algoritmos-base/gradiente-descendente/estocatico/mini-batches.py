...
def gradiente_descendente(w, X, y, tx_aprendizado, num_epocas, batch_size=32):
    ...
    for epoca in range(num_epocas):
        ...
        for inicio in range(0, len(X), batch_size):
            fim = min(inicio + batch_size, len(X))
            X_batch = X_embaralhado[inicio:fim]
            y_batch = y_embaralhado[inicio:fim]

            sigmoid_res = sigmoid(X_batch @ w.T)
            derivada_custo = np.sum((sigmoid_res - y_batch) * X_batch, axis=0)
            w = w - (tx_aprendizado / len(X_batch)) * derivada_custo

        custo_por_epoca[epoca] = maxima_verossimilhanca(w, X, y)
...
batch_size = 32
pesos, lista_custos = gradiente_descendente(w, X, y, tx_aprendizado, total_iteracoes, batch_size=batch_size)
