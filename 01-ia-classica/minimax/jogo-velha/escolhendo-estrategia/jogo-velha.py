...
class JogoVelha: # IA é sempre O e o jogador é sempre X
  ...
  def play_ia(self):
    ...
    proxima_jogada = self.definir_proxima_jogada(jogadas)
    self.tabuleiro[proxima_jogada[0]][proxima_jogada[1]] = 'O'

  # Reescrever pela estrategia a ser usada no set_strategy
  def definir_proxima_jogada(self, jogadas: List[Position]) -> Position:
    return jogadas[0]

  # Definir a estratégia, setando métodos e variáveis extras (incluindo a definicao do definir_proxima_jogada)
...
jogo = JogoVelha()
# jogo.set_strategy = types.MethodType(randStrat.setRandomStrat, jogo)
jogo.set_strategy = types.MethodType(minimaxStrat.setMinimaxStrat, jogo)
jogo.set_strategy()
